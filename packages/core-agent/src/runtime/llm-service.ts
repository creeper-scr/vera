 ...
 *
 * Expects:
 * - `role: 'error'` entries (Vera-internal markers from the chat UI). They are
 ...
 *
 * @param messages - Raw messages from the chat session, may include Vera's
 *   `error` role.
...
export function sanitizeMessages(messages: unknown[], supportsContentArray: boolean = true): Message[] {
  return messages.map((message: any) => {
    ...
    // string variant.
    // Source/context: https://github.com/moeru-ai/vera/issues/1500
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    try {
      const streamResult = streamText({
        ...
        stopWhen: stepCountAtLeast(options?.maxSteps ?? 10),
        // NOTICE:
        ...
        // Vera captures tool failures by wrapping local tool executors instead.
        tools: streamTools,
        onEvent,
      })
...
//
// See: https://github.com/moeru-ai/vera/issues/1500
