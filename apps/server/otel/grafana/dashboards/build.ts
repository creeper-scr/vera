/**
 * Dashboard generator for `vera-server-overview-cloud.json`.
 *
 * Run: `pnpm -F @proj-vera/server otel:dashboards`
...
const PROM = { name: 'grafanacloud-projvera-prom' }
const LOKI = { name: 'grafanacloud-projvera-logs' }
...
elements['panel-95'] = statPanel(
  ...
  [
    query(`sum(increase(vera_product_events_total{${PRODUCT_EVENT_FILTER}}[$__range]))`, 'events'),
  ],
...
elements['panel-96'] = gaugePanel(
  ...
  [query(
    `100 * sum(increase(vera_product_events_total{${PRODUCT_EVENT_FILTER}, status="failed"}[$__range])) / clamp_min(sum(increase(vera_product_events_total{${PRODUCT_EVENT_FILTER}}[$__range])), 1)`,
    'failed %',
  )],
...
elements['panel-97'] = barGaugePanel(
  ...
  [query(
    `topk(12, sum by (feature, action, status) (increase(vera_product_events_total{${PRODUCT_EVENT_FILTER}}[$__range])))`,
...
elements['panel-98'] = timeseriesPanel(
  ...
  [query(
    `sum by (feature, action, status) (rate(vera_product_events_total{${PRODUCT_EVENT_FILTER}}[$__rate_interval]))`,
    '{{feature}} · {{action}} · {{status}}',
  )],
...
elements['panel-69'] = timeseriesPanel(
  ...
  'Billed TTS characters per second by model (from `vera.billing.tts.chars`). The 用量统计「字数」screen on the TTS consoles (豆包 / 阿里), unified. Integrate over the range for a window total.',
  [query(
    `sum by (model) (rate(vera_billing_tts_chars_total{${SERVICE_FILTER}}[$__rate_interval]))`,
    '{{model}}',
  )],
  { unit: 'short' },
)
...
elements['panel-43'] = statPanel(
  ...
  [query(
    `sum(increase(vera_billing_flux_unbilled_total{${SERVICE_FILTER}, reason!="partial_debit_drained"}[$__range]))`,
    'flux',
  )],
...
elements['panel-41'] = statPanel(
  ...
  [query(
    `sum(increase(vera_gen_ai_stream_interrupted_total{${SERVICE_FILTER}}[$__range]))`,
    'interruptions',
  )],
...
// `_total` for counters): vera_gen_ai_gateway_{key_exhausted,decrypt_failures,
...
elements['panel-60'] = statPanel(
  ...
  [query(`sum(increase(vera_gen_ai_gateway_key_exhausted_total{${SERVICE_FILTER}}[5m]))`, 'events')],
  { unit: 'short', steps: [{ color: 'green', value: 0 }, { color: 'red', value: 1 }], noValue: '0', graphMode: 'none' },
)
...
elements['panel-61'] = statPanel(
  ...
  [query(`sum(increase(vera_gen_ai_gateway_decrypt_failures_total{${SERVICE_FILTER}}[5m]))`, 'events')],
  { unit: 'short', steps: [{ color: 'green', value: 0 }, { color: 'red', value: 1 }], noValue: '0', graphMode: 'none' },
)
...
elements['panel-62'] = gaugePanel(
  ...
  [query(
    `100 * sum(rate(vera_gen_ai_gateway_fallback_count_total{${SERVICE_FILTER}}[5m])) / clamp_min(sum(rate(gen_ai_client_operation_count_total{${SERVICE_FILTER}}[5m])), 1)`,
    'fallback %',
  )],
...
elements['panel-65'] = timeseriesPanel(
  ...
  [query(
    `sum by (provider, status_code) (rate(vera_gen_ai_gateway_upstream_errors_total{${SERVICE_FILTER}}[$__rate_interval]))`,
...
elements['panel-30'] = statPanel(
  ...
  [query(
    `sum by (currency) (increase(vera_stripe_revenue_minor_unit_total{${SERVICE_FILTER}, currency!=""}[$__range])) / 100`,
    '{{currency}}',
  )],
...
/**
 * Vera Server Overview dashboard.
 *
 * Reading order:
...
export const dashboard = {
  ...
  liveNow: false,
  preload: false,
  tags: ['vera', 'observability', 'grafana-cloud'],
  timeSettings: {
    autoRefresh: '',
...
function writeDashboard(): void {
  ...
  const outPath = join(here, 'vera-server-overview-cloud.json')
