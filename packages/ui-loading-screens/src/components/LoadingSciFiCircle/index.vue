...
import CircleFadeInAnimation from './assets/circle_blink_in_-_loading_(@proj-vera).riv'
...
const projectVeraAsciiArt = computed(() => {
  if (isDeviceSm.value)
    return narrowAsciiArt.value
  return wideAsciiArt.value
})

const projectVeraMetadata = `
Project Vera team from Moeru AI (https://moeru.ai) and other contributors
Open sourced on https://github.com/moeru-ai/vera
`

const bootMessages = computed<BootMessage[]>(() => [
  ...projectVeraAsciiArt.value.split('\n').map(line => ({
    template: line,
    typingSpeed: 1,
    withoutTimestamp: true,
  })),
  {
    template: `Project Vera version ${import.meta.env.VITE_VERA_VERSION || '1.0.0'} @ ${import.meta.env.VITE_VERA_COMMIT || '0240602'} build`,
    typingSpeed: 5,
    withoutTimestamp: true,
  },
  ...projectVeraMetadata.trim().split('\n').map(line => ({
    template: line,
    typingSpeed: 1,
    withoutTimestamp: true,
  })),
  ...
  {
    template: 'Command line: BOOT_IMAGE=/boot/vera.moeru.ai root=UUID=io.github.moeru-ai.vera',
    typingSpeed: 1,
  },
  {
  ...
  {
    template: 'Initializing Vera subsystems...',
    typingSpeed: 1,
    pending: true,
    onPendingCheck: () => {
  ...
  {
    template: 'Vera core services initialized',
    typingSpeed: 1,
  },
  {
