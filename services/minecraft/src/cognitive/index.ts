...
import { createMinecraftGameAdapter } from '../game-coop/minecraftGameAdapter'
import { MinecraftGameCoopChannel } from '../game-coop/minecraftGameCoopChannel'
...
export function CognitiveEngine(options: CognitiveEngineOptions): MineflayerPlugin {
  ...
  let gameCoopChannel: MinecraftGameCoopChannel | null = null
  let started = false

  return {
    async created(bot) {
      // Create container and get required services
      container = createAgentContainer(options.veraClient)
      ...
      const veraBridge = container.resolve('veraBridge')
      ...
      const startCognitive = () => {
        ...
        const gameAdapter = createMinecraftGameAdapter(
          botWithAgents,
          reflexManager,
          taskExecutor,
          config.bot.masterUsername,
        )
        options.onGameAdapter?.(gameAdapter)
        gameCoopChannel = new MinecraftGameCoopChannel({
          client: options.veraClient,
          adapter: gameAdapter,
        })
        gameCoopChannel.init()
    ...
    async beforeCleanup(bot) {
      gameCoopChannel?.destroy()
      gameCoopChannel = null
