...
Ping API 会检查网络、模型列表和聊天请求。出现权限或模型不可用错误时，请先检查 API Key 对应项目的 API 启用状态与地区可用性。不要把 Google AI Studio 中展示的模型名称改写为其他格式；在 Vera 中优先从模型列表选择。
