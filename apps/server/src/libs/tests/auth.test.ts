...
describe('createAuth', () => {
  ...
  it('does not register social providers without credentials', () => {
    const auth = createAuth({} as unknown as Database, {
      API_SERVER_URL: 'http://localhost:3000',
...
describe('seedTrustedClients', () => {
  it('seeds trusted first-party clients with explicit oauth metadata', async () => {
    ...
    expect(webClient.clientId).toBe('vera-stage-web')
    ...
    expect(webClient.redirectUris).toEqual([
      'https://vera.moeru.ai/auth/callback',
    ...
    expect(electronClient.clientId).toBe('vera-stage-electron')
    ...
    expect(pocketClient.clientId).toBe('vera-stage-pocket')
    ...
    expect(pocketClient.redirectUris).toEqual([
      ...
      'ai.moeru.vera-pocket://links/auth/callback',
    ])
  })
...
describe('ensureDynamicFirstPartyRedirectUri', () => {
  it('appends a trusted web callback redirect URI discovered from the authorize request', async () => {
        ...
        from: vi.fn(() => ({
          where: vi.fn(() => ({
            limit: vi.fn().mockResolvedValue([
              { redirectUris: ['https://vera.moeru.ai/auth/callback'] },
            ]),
          })),
        })),
      })),
    ...
    await ensureDynamicFirstPartyRedirectUri(
      db as any,
      new Request('https://api.vera.build/api/auth/oauth2/authorize?client_id=vera-stage-web&redirect_uri=https%3A%2F%2Fpreview.kwaa.workers.dev%2Fauth%2Fcallback'),
      [],
    )
  ...
  it('appends a same-origin electron relay redirect URI discovered from the authorize request', async () => {
    ...
    const db = {
      select: vi.fn(() => ({
        from: vi.fn(() => ({
          where: vi.fn(() => ({
            limit: vi.fn().mockResolvedValue([
              { redirectUris: ['https://api.vera.build/api/auth/oidc/electron-callback'] },
            ]),
          })),
        })),
      })),
    ...
    await ensureDynamicFirstPartyRedirectUri(
      db as any,
      new Request('https://vera-server-dev.up.railway.app/api/auth/oauth2/authorize?client_id=vera-stage-electron&redirect_uri=https%3A%2F%2Fvera-server-dev.up.railway.app%2Fapi%2Fauth%2Foidc%2Felectron-callback'),
      [],
    )

    expect(setCalls).toHaveLength(1)
    expect(setCalls[0].redirectUris).toEqual([
      'https://api.vera.build/api/auth/oidc/electron-callback',
      'https://vera-server-dev.up.railway.app/api/auth/oidc/electron-callback',
    ])
    expect(updateWhere).toHaveBeenCalledTimes(1)
  })
  ...
  it('ignores untrusted or non-callback redirect URIs', async () => {
    ...
    await ensureDynamicFirstPartyRedirectUri(
      db as any,
      new Request('https://api.vera.build/api/auth/oauth2/authorize?client_id=vera-stage-web&redirect_uri=https%3A%2F%2Fevil.example%2Fauth%2Fcallback'),
      [],
    )

    await ensureDynamicFirstPartyRedirectUri(
      db as any,
      new Request('https://api.vera.build/api/auth/oauth2/authorize?client_id=vera-stage-web&redirect_uri=https%3A%2F%2Fvera.moeru.ai%2Fother-path'),
      [],
    )

    await ensureDynamicFirstPartyRedirectUri(
      db as any,
      new Request('https://api.vera.build/api/auth/oauth2/authorize?client_id=vera-stage-electron&redirect_uri=https%3A%2F%2Fother.example%2Fapi%2Fauth%2Foidc%2Felectron-callback'),
      [],
    )
