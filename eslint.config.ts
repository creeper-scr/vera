...
export default defineConfig({
  ...
  rules: {
    ...
    'no-restricted-syntax': [
      'warn',
      {
        ...
        message: 'Avoid `error instanceof Error ? error.message : ...`. Use `errorMessageFrom(error)` from \'@moeru/std\' (or `errorMessageFromUnknown(error, fallback)` from \'@proj-vera/stage-shared\'). Pair with `?? \'fallback\'` when a default is needed.',
      },
  ...
  rules: {
    'no-restricted-syntax': [
      'error',
      {
        selector: 'CallExpression[callee.type=\'MemberExpression\'][callee.object.name=\'vi\'][callee.property.name=/^(mock|doMock)$/][arguments.0.type=\'Literal\'][arguments.0.value=/^(\\.|@proj-vera\\/|~)/]',
