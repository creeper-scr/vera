...
const VS_DECL = `
#ifndef VERA_DIFFUSE_VS_DECL
#define VERA_DIFFUSE_VS_DECL
varying vec3 vWorldNormal;
#endif
`

const VS_APPLY = `
#ifndef VERA_DIFFUSE_VS_APPLY
#define VERA_DIFFUSE_VS_APPLY
vWorldNormal = normalize( mat3( modelMatrix ) * objectNormal );
#endif
`

const FS_COMMON = `
#ifndef VERA_DIFFUSE_COMMON
#define VERA_DIFFUSE_COMMON
...
vec3 VERA_evalIrradianceSH(vec3 n){
  n = normalize(n);
...
const FS_APPLY = `
#ifndef VERA_DIFFUSE_APPLY
#define VERA_DIFFUSE_APPLY
if (uNprEnvMode == 2) {
  vec3 I = VERA_evalIrradianceSH(normalize(vWorldNormal));
...
export function injectDiffuseIBL(mat: THREE.ShaderMaterial) {
  ...
  mat.customProgramCacheKey = () => `${baseKey}|vera-diffuse-ibl`
  ...
  mat.onBeforeCompile = (shader: any, renderer: any) => {
    ...
    if (!shader.vertexShader.includes('VERA_DIFFUSE_VS_DECL')) {
      shader.vertexShader = `${VS_DECL}\n${shader.vertexShader}`
    }
    if (shader.vertexShader.includes('#include <defaultnormal_vertex>')
      && !shader.vertexShader.includes('VERA_DIFFUSE_VS_APPLY')) {
    ...
    if (!shader.fragmentShader.includes('VERA_DIFFUSE_COMMON')) {
    ...
    if (!shader.fragmentShader.includes('VERA_DIFFUSE_APPLY')) {
      shader.fragmentShader = shader.fragmentShader.replace(
    ...
    (mat.userData ||= {}).__veraIbl = shader.uniforms
  }
