...
export async function validateLive2DZip(file: File | Blob): Promise<Live2DValidationReport> {
  ...
  // 3. Basename Collision Audit (Vera ZipLoader weakness)
  ...
  for (const [base, paths] of basenames.entries()) {
    if (paths.length > 1) {
      report.errors.push(`BASENAME COLLISION: Filename "${base}" exists in multiple locations: ${paths.join(', ')}. This causes data loss in Vera's loader.`)
    }
  }
