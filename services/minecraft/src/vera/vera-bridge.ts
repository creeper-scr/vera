import type { Client, ContextUpdate, ModuleAnnouncedEvent } from '@proj-vera/server-sdk'

import type { EventBus } from '../cognitive/event-bus'

import { useLogg } from '@guiiai/logg'
import { ContextUpdateStrategy } from '@proj-vera/server-sdk'
...
export class VeraBridge {
  private readonly logger = useLogg('vera-bridge').useGlobalConfig()
  ...
  init(): void {
    this.commandHandler = (event) => {
      ...
      // A spark:command is high-level guidance from the Vera server. It must carry enough weight to
      // trigger a fresh decision (Conscious) cycle, never be silently filed into history — so we
      // always route it through handleActionIntent (→ signal:vera_command → enqueueEvent → decision cycle).
      //
      // We intentionally do not special-case `intent === 'context'`: that branch used to emit
      // signal:vera_context which Brain pushes to conversationHistory WITHOUT waking the loop, so a
    ...
    this.contextUpdateHandler = (event) => {
      ...
      this.eventBus.emit({
        type: 'signal:vera_context',
        payload: Object.freeze({
          type: 'vera_context' as const,
        ...
        source: { component: 'vera', id: 'bridge' },
      })
    }
    ...
    this.logger.log('VeraBridge initialized, listening for spark:command, context:update, and module:announced')
  }
  ...
  destroy(): void {
    ...
    this.logger.log('VeraBridge destroyed')
  }
  ...
  private handleActionIntent(cmd: SparkCommandData): void {
    // A spark:command is high-level guidance from the Vera server. Route it through the explicit
    // `vera_command` signal so the brain runs a fresh decision cycle
    // (resetNoActionFollowupBudget('vera_command'), normal Conscious wake-up) instead of silently
    // filing it into history. The directive is attributed to the Vera server as a neutral source,
    ...
    this.logger.log('Routing spark:command as a Vera directive', {
    ...
    this.eventBus.emit({
      ...
      payload: Object.freeze({
        type: 'vera_command' as const,
        description: `Directive from Vera: "${message}"`,
        sourceId,
        confidence: 1.0,
        timestamp: Date.now(),
        metadata: {
          message,
          // Keep the spark provenance for debugging; the brain sees a typed Vera directive.
