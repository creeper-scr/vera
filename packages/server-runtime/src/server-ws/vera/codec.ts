import type { WebSocketBaseEvent, WebSocketEvent } from '@proj-vera/server-shared/types'

import { MessageHeartbeatKind } from '@proj-vera/server-shared/types'
import { parse, stringify } from 'superjson'
import { check, objectWithRest, pipe, safeParse, string, unknown } from 'valibot'

const invalidVeraWebSocketEventFormatMessage = 'Invalid WebSocket event format.'
...
/** Error thrown when parsed websocket text is not an Vera event envelope. */
export class InvalidEventError extends Error {
  readonly source?: unknown

  constructor(options: InvalidEventErrorOptions = {}) {
    super(invalidVeraWebSocketEventFormatMessage, { cause: options.cause })
    this.name = 'InvalidEventError'
    this.source = options.source
  }
}

/** Checks whether an error came from Vera websocket event envelope validation. */
...
/** Parses one Vera websocket protocol event from SuperJSON or plain JSON text. */
export function parseEvent(text: string): WebSocketEvent {
  ...
  // Keep this until all Vera websocket clients share one non-wrapper wire format.
...
/** Serializes one Vera websocket protocol event with the existing SuperJSON wire format. */
