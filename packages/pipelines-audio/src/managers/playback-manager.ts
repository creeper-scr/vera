...
export function createPlaybackManager<TAudio>(
  ...
  function tryStartWaiting() {
    ...
    while (i < waiting.length && active.size < maxVoices) {
      ...
      const stealsActivePlayback
        = policy === 'steal-oldest' || policy === 'steal-lowest-priority'
      if (stealsActivePlayback && hasStolenDuringWake) {
        // Fill genuinely free voices, but do not turn one completion into a
        // chain of unrelated active-playback interruptions.
        i++
        continue
      }
