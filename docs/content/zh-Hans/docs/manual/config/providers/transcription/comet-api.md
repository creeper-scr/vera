...
description: 在 Vera 中配置 CometAPI 语音识别
---

CometAPI 通过其兼容接口提供音频转写。

::: info 为什么选择 CometAPI？
如果你已使用 CometAPI 管理模型和凭据，可在 Vera 中直接复用同一 API Key 进行语音识别。
:::
...
Ping API 失败时，检查 API Key、账户权限和网络连接。没有文字结果时，确认 Vera 已获得系统麦克风权限。
