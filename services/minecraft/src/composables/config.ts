...
export const configSchema = z.object({
  ...
  vera: z.object({
    wsBaseUrl: wsUrlString('VERA_WS_BASEURL'),
    clientName: requiredString('VERA_CLIENT_NAME'),
...
export function initEnv(): void {
  ...
  const parsedConfig = configSchema.safeParse({
    ...
    vera: {
      wsBaseUrl: env.VERA_WS_BASEURL ?? defaultConfig.vera.wsBaseUrl,
      clientName: env.VERA_CLIENT_NAME ?? defaultConfig.vera.clientName,
      token: env.VERA_WS_TOKEN || defaultConfig.vera.token,
    },
  })
  ...
  config.vera = parsedConfig.data.vera
  ...
  logger.withFields({
    config: {
      ...
      vera: { ...config.vera, token: '[REDACTED]' },
    },
