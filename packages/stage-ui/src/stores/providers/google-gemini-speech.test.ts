...
describe('googleGeminiSpeech request construction', () => {
  ...
  it('fetch adapter sends correct Gemini request body', async () => {
    ...
    await fetchFn(new URL('http://test'), {
      body: JSON.stringify({ model: 'gemini-2.5-flash-preview-tts', input: 'Hello from Vera', voice: 'Kore' }),
    })
    ...
    expect(requestBody.contents[0].parts[0].text).toBe('Hello from Vera')
  })
