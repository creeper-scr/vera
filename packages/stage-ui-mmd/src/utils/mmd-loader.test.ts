...
describe('withModelExtension', () => {
  ...
  it('tags an extensionless blob URL with the known format (Issue: blob import)', () => {
    expect(withModelExtension('blob:http://host/9f1c-abc', 'pmx')).toBe('blob:http://host/9f1c-abc#vera-model.pmx')
    expect(withModelExtension('blob:http://host/9f1c-abc', 'pmd')).toBe('blob:http://host/9f1c-abc#vera-model.pmd')
  })
