...
import { Button, Callout, FieldCheckbox, FieldCombobox } from '@proj-vera/ui'
...
const props = withDefaults(defineProps<{
  ...
  /** Whether a speech-to-text provider is selected. Undefined hides provider readiness UI. */
  transcriptionReady?: boolean
  /** Whether a text-to-speech provider is selected. Undefined hides provider readiness UI. */
  speechReady?: boolean
}>(), {
  granted: false,
})
...
function toggleHearingEnabled() {
  ...
  if (props.transcriptionReady === false) {
    emit('configureTranscription')
    return
  }
...
<template>
  <div class="space-y-2">
    <div
      v-if="props.transcriptionReady !== undefined || props.speechReady !== undefined"
      :class="[
        'mb-3 rounded-xl border border-neutral-200 p-3 dark:border-neutral-800',
        'bg-neutral-50/80 dark:bg-neutral-950/35',
      ]"
    >
      ...
      <div v-if="props.transcriptionReady !== undefined" class="flex items-center gap-3">
        <div
          :class="[
            'size-2 shrink-0 rounded-full',
            props.transcriptionReady ? 'bg-green-500' : 'bg-amber-500',
          ]"
        />
        <div class="min-w-0 flex-1">
          ...
          <div class="text-xs text-neutral-500 dark:text-neutral-400">
            {{ props.transcriptionReady ? 'Ready' : 'Setup required before listening' }}
          </div>
        </div>
        <Button
          v-if="!props.transcriptionReady"
          label="Configure STT"
          size="sm"
          variant="secondary"
          @click="emit('configureTranscription')"
        />
      </div>

      <div v-if="props.speechReady !== undefined" class="mt-3 flex items-center gap-3">
        <div
          :class="[
            'size-2 shrink-0 rounded-full',
            props.speechReady ? 'bg-green-500' : 'bg-neutral-400',
          ]"
        />
        <div class="min-w-0 flex-1">
          <div class="text-sm font-medium">
          ...
          <div class="text-xs text-neutral-500 dark:text-neutral-400">
            {{ props.speechReady ? 'Ready' : 'Off; game actions still run' }}
          </div>
        </div>
        <Button
          v-if="!props.speechReady"
          label="Configure TTS"
          size="sm"
          variant="secondary"
          @click="emit('configureSpeech')"
        />
      </div>
    </div>
