...
import type { VeraCard } from '@proj-vera/stage-ui/stores/modules/vera-card'

import DOMPurify from 'dompurify'

import { useAnalytics } from '@proj-vera/stage-ui/composables'
import { useDownload } from '@proj-vera/stage-ui/composables/download'
import { exportVeraCardPackage } from '@proj-vera/stage-ui/services/vera-card-import-export'
import { useBackgroundStore } from '@proj-vera/stage-ui/stores/background'
...
import { Button, Select } from '@proj-vera/ui'
...
const selectedCard = computed<VeraCard | undefined>(() => {
  if (!props.cardId)
    return undefined
...
const moduleSettings = computed(() => {
  if (!selectedCard.value || !selectedCard.value.extensions?.vera?.modules) {
    return {
  ...
  const veraExt = selectedCard.value.extensions.vera.modules
  return {
    consciousnessProvider: veraExt.consciousness?.provider || '',
    consciousness: veraExt.consciousness?.model || '',
    visionProvider: veraExt.vision?.provider || '',
    vision: veraExt.vision?.model || '',
    speechProvider: veraExt.speech?.provider || '',
    speech: veraExt.speech?.model || '',
    voice: veraExt.speech?.voice_id || '',
  }
})
...
async function handleExportCard() {
  ...
  try {
    useDownload(
      await exportVeraCardPackage({ card: selectedCard.value, displayModelsStore }),
...
const activeBackgroundId = computed({
  get: () => selectedCard.value?.extensions?.vera?.modules?.activeBackgroundId || 'none',
  set: async (val: string) => {
    if (!selectedCard.value)
      return
    const extension = JSON.parse(JSON.stringify(selectedCard.value.extensions))
    if (!extension.vera.modules)
      extension.vera.modules = {}

    extension.vera.modules.activeBackgroundId = val
