...
import type { MinecraftContextService } from '../../vera/minecraft-context-service'
import type { VeraBridge } from '../../vera/vera-bridge'
 ...
 * Player chat and Vera commands always take precedence over stale system feedback.
 */
...
function getEventPriority(event: BotEvent): number {
  if (event.type === 'perception') {
    ...
    if (signal.type === 'chat_message' || signal.type === 'vera_command')
...
export class Brain {
  ...
  public init(bot: MineflayerWithAgents): void {
    ...
    this.unsubscribeEventBus = this.deps.eventBus.subscribe<PerceptionSignal>('conscious:signal:*', (event: TracedEvent<PerceptionSignal>) => {
      // Vera context updates are injected into conversation history without triggering a full cognitive cycle
      if (event.payload.type === 'vera_context') {
        this.conversationHistory.push({
          role: 'user',
          content: `[VERA_CONTEXT] ${event.payload.description}`,
        })
        this.deps.logger.log('INFO', `Brain: Injected Vera context: ${event.payload.description.slice(0, 80)}`)
        return
      }
      ...
      this.enqueueEvent(bot, {
        ...
        source: { type: event.payload.sourceId === 'vera' ? 'vera' : 'minecraft', id: event.payload.sourceId ?? 'perception' },
        timestamp: Date.now(),
  ...
  private createRuntimeGlobals(
    ...
    return {
      ...
      notifyVera: (headline: string, note?: string, urgency?: 'immediate' | 'soon' | 'later') =>
        this.deps.veraBridge.sendNotify(headline, note, urgency),
      updateVeraContext: (text: string, hints?: string[], lane?: string) =>
        this.deps.veraBridge.sendContextUpdate(text, hints, lane),
    }
  }
  ...
  private isVeraCommandEvent(event: BotEvent): boolean {
    if (event.type !== 'perception')
      return false
    const signal = event.payload as PerceptionSignal
    return signal.type === 'vera_command'
  }
  ...
  private resetNoActionFollowupBudget(reason: 'player_chat' | 'manual' | 'vera_command'): NoActionBudgetState {
  ...
  /**
   * Coalesce the event queue: promote high-priority events (player chat, Vera commands)
  ...
  private async processEvent(bot: MineflayerWithAgents, event: BotEvent): Promise<void> {
    ...
    if (this.isVeraCommandEvent(event))
      this.resetNoActionFollowupBudget('vera_command')
  ...
  private canResumeFromGiveUp(signal: PerceptionSignal): boolean {
    return signal.type === 'chat_message' || signal.type === 'vera_command'
  }
}
