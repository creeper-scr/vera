...
describe('javaScriptPlanner', () => {
  ...
  it('bridges Vera notification callbacks through the isolate', async () => {
    const planner = new JavaScriptPlanner()
    const executeAction = vi.fn(async action => `ok:${action.tool}`)
    const notifyVera = vi.fn()
    const updateVeraContext = vi.fn()

    const planned = await planner.evaluate(`
      notifyVera('Need help', 'Low health', 'immediate')
      updateVeraContext('Shelter built', ['shelter', 'spawn'], 'memory')
      return 'ok'
    `, actions, {
      ...globals,
      notifyVera,
    ...
    expect(notifyVera).toHaveBeenCalledWith('Need help', 'Low health', 'immediate')
    expect(updateVeraContext).toHaveBeenCalledWith('Shelter built', ['shelter', 'spawn'], 'memory')
  })
