...
describe('isToolRelatedError', () => {
  ...
  it('streams isolated action requests with only their explicit tools', async () => {
    const store = useLLM()
    const llmToolsStore = useLlmToolsStore()
    const gameTool = {
      type: 'function',
    ...
    llmToolsStore.registerTools('plugin-tools', [{
      ...gameTool,
      function: { ...gameTool.function, name: 'unrelated-runtime-tool' },
    }])
    streamTextMock.mockReturnValueOnce(createMockStreamResult())

    await store.streamWithExclusiveTools(
      'model-a',
      provider,
      [{ role: 'user', content: 'move' }] as Message[],
      { tools: [gameTool], maxSteps: 1 },
    )

    expect(streamTextMock.mock.calls[0]?.[0]?.tools).toEqual([gameTool])
