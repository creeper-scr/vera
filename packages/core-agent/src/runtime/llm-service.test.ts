...
describe('streamFrom tool error capture', () => {
  it('applies a request-scoped model step limit', async () => {
    const stopCondition = vi.fn()
    stepCountAtLeastMock.mockReturnValueOnce(stopCondition)
    ...
    expect(stepCountAtLeastMock).toHaveBeenCalledWith(1)
