...
import type { Card } from '@proj-vera/ccc'
import type { VeraExtension } from '@proj-vera/stage-ui/stores/modules/vera-card'

import kebabcase from '@stdlib/string-base-kebabcase'

import { isCustomProvidersDisabled } from '@proj-vera/stage-shared'
import { useAnalytics } from '@proj-vera/stage-ui/composables'
import { DEFAULT_ARTISTRY_WIDGET_INSTRUCTION } from '@proj-vera/stage-ui/constants/prompts/artistry-instruction'
import { useDisplayModelsStore } from '@proj-vera/stage-ui/stores/display-models'
import { useArtistryStore } from '@proj-vera/stage-ui/stores/modules/artistry'
import { useConsciousnessStore } from '@proj-vera/stage-ui/stores/modules/consciousness'
import { useSpeechStore } from '@proj-vera/stage-ui/stores/modules/speech'
import { useVeraCardStore } from '@proj-vera/stage-ui/stores/modules/vera-card'
import { useVisionStore } from '@proj-vera/stage-ui/stores/modules/vision'
import { useProvidersStore } from '@proj-vera/stage-ui/stores/providers'
import { useSettingsStageModel } from '@proj-vera/stage-ui/stores/settings/stage-model'
import { Button, FieldInput, FieldValues } from '@proj-vera/ui'
import { ComboboxSelect } from '@proj-vera/ui/components/form'
...
type VeraExtensionWithLegacyArtistry = VeraExtension & {
  artistry?: LegacyArtistrySettings
  modules?: VeraExtension['modules'] & {
    artistry?: LegacyArtistrySettings
  }
}
...
function initializeCard(): Card {
  ...
  const veraExt = existingCard?.extensions?.vera as VeraExtensionWithLegacyArtistry | undefined

  // Initialize module selections with fallback logic (handles all cases: create, edit with/without extension)
  selectedConsciousnessProvider.value = veraExt?.modules?.consciousness?.provider || consciousnessProvider.value
  selectedConsciousnessModel.value = veraExt?.modules?.consciousness?.model || defaultConsciousnessModel.value
  selectedVisionProvider.value = veraExt?.modules?.vision?.provider || visionProvider.value
  selectedVisionModel.value = veraExt?.modules?.vision?.model || defaultVisionModel.value
  selectedSpeechProvider.value = veraExt?.modules?.speech?.provider || speechProvider.value
  selectedSpeechModel.value = veraExt?.modules?.speech?.model || defaultSpeechModel.value
  selectedSpeechVoiceId.value = veraExt?.modules?.speech?.voice_id || defaultSpeechVoiceId.value
  selectedDisplayModelId.value = veraExt?.modules?.displayModelId || defaultDisplayModelId.value

  // NOTICE: keep legacy `extensions.vera.artistry` fallback so existing cards continue to load.
  const artistrySettings = veraExt?.modules?.artistry || veraExt?.artistry
