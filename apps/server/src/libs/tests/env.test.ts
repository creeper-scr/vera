...
describe('parseEnv', () => {
  it('parses the required auth and infrastructure environment variables', () => {
    ...
    expect(env.AUTH_UI_URL).toBe('https://accounts.vera.build/ui')
    expect(env.ADMIN_UI_URL).toBe('https://admin.vera.build')
    expect(env.ADDITIONAL_TRUSTED_ORIGINS).toEqual([])
  })

  it('allows local runtime without social OAuth credentials', () => {
    const { AUTH_GOOGLE_CLIENT_ID: _googleId, AUTH_GOOGLE_CLIENT_SECRET: _googleSecret, AUTH_GITHUB_CLIENT_ID: _githubId, AUTH_GITHUB_CLIENT_SECRET: _githubSecret, ...withoutSocialOAuth } = baseEnv()
    const env = parseEnv(withoutSocialOAuth)

    expect(env.AUTH_GOOGLE_CLIENT_ID).toBe('')
