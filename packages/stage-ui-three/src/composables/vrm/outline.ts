...
export const VERA_OUTLINE_NORMAL_ATTRIBUTE_NAME = 'outlineNormal'
export const VERA_OUTLINE_PREPROCESS_VERSION = 1
export const VERA_OUTLINE_SHADER_PATCH_VERSION = 1

const VERA_OUTLINE_PREPROCESS_USER_DATA_KEY = '__veraOutlinePreprocess'
const VERA_OUTLINE_SHADER_PATCH_USER_DATA_KEY = '__veraOutlineShaderPatch'
const VERA_OUTLINE_SHADER_PATCH_CACHE_KEY = `veraOutline:viewspace-v${VERA_OUTLINE_SHADER_PATCH_VERSION}`
...
const VERA_OUTLINE_VIEW_FLATTENED_Z = -0.1
const VERA_OUTLINE_VIEW_Z_OFFSET = -0.001
...
function isOutlineGeometryPreprocessed(geometry: BufferGeometry) {
  return hasPatchedState(
    geometry,
    VERA_OUTLINE_PREPROCESS_USER_DATA_KEY,
    VERA_OUTLINE_PREPROCESS_VERSION,
    geometryStateRegistry,
    () => geometry.getAttribute(VERA_OUTLINE_NORMAL_ATTRIBUTE_NAME) != null,
  )
}
...
function markOutlineGeometryPreprocessed(geometry: BufferGeometry) {
  markPatchedState(
    ...
    VERA_OUTLINE_PREPROCESS_VERSION,
    geometryStateRegistry,
    { attribute: VERA_OUTLINE_NORMAL_ATTRIBUTE_NAME },
  )
}
...
function ensureOutlineNormalAttribute(geometry: BufferGeometry) {
  ...
  geometry.setAttribute(VERA_OUTLINE_NORMAL_ATTRIBUTE_NAME, new Float32BufferAttribute(outlineNormals, 3))
...
function isOutlineMaterialPatched(material: Material) {
  return hasPatchedState(
    material,
    VERA_OUTLINE_SHADER_PATCH_USER_DATA_KEY,
    VERA_OUTLINE_SHADER_PATCH_VERSION,
...
function injectOutlineNormalAttribute(vertexShader: string) {
  return vertexShader.replace(
    ...
    `${SHADER_COMMON_ANCHOR}
attribute vec3 ${VERA_OUTLINE_NORMAL_ATTRIBUTE_NAME};`,
  )
}
...
function replaceOutlineNormalSource(vertexShader: string) {
  return vertexShader.replace(
    ...
    `${SHADER_BEGINNORMAL_ANCHOR}
  objectNormal = ${VERA_OUTLINE_NORMAL_ATTRIBUTE_NAME};`,
  )
}
...
function replaceOutlineExtrusionBlock(vertexShader: string) {
  return vertexShader.replace(
    ...
    outlineDirectionVS.z = ${VERA_OUTLINE_VIEW_FLATTENED_Z.toFixed(1)};
    ...
    outlinePositionVS.z += ${VERA_OUTLINE_VIEW_Z_OFFSET.toFixed(3)};
...
function patchBuiltInOutlineMaterial(material: MToonMaterial) {
  ...
  if (!patchedVertexShader) {
    console.warn(
      '[Vera] Failed to patch built-in MToon outline shader: expected shader anchors were not found.',
      material.name,
    )
    return false
  }
  ...
  material.customProgramCacheKey = () => {
    ...
    return baseKey
      ? `${baseKey},${VERA_OUTLINE_SHADER_PATCH_CACHE_KEY}`
      : VERA_OUTLINE_SHADER_PATCH_CACHE_KEY
  }
...
export function createVrmOutlineHook(): VrmHook {
  return {
    ...
    onMaterial(context) {
      ...
      if (!meshTarget.geometry.getAttribute(VERA_OUTLINE_NORMAL_ATTRIBUTE_NAME)) {
        console.warn(
          '[Vera] Failed to patch built-in MToon outline shader: outlineNormal is missing on geometry.',
