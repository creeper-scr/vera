...
```sh
pnpm -F @proj-vera/server dev                # dev with dotenvx (.env.local)
pnpm -F @proj-vera/server typecheck
pnpm -F @proj-vera/server exec vitest run    # all server tests
pnpm exec vitest run apps/server/src/...     # single test file
pnpm -F @proj-vera/server db:generate        # drizzle-kit generate
pnpm -F @proj-vera/server db:push            # drizzle-kit push
pnpm -F @proj-vera/server auth:generate      # better-auth → src/schemas/accounts.ts
```
...
- **Schemas** (`src/schemas/`): Drizzle table definitions. Migrations in `@proj-vera/server-schema`.
