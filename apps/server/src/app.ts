...
import { createRealtimeVoiceWsHandlers } from './routes/realtime-voice-ws'
...
export async function buildApp(deps: AppDeps) {
  ...
  const realtimeVoiceWsSetup = createRealtimeVoiceWsHandlers()
  app.get('/api/v1/audio/realtime/ws', upgradeWebSocket(async (c) => {
    const token = c.req.query('token')
    if (!token)
      return createUnauthorizedWsEvents()
    ...
    return realtimeVoiceWsSetup(session.user.id)
  }))
  ...
  const builtApp = app
    ...
    .on('GET', '/', c => c.json({
      service: 'vera-api',
      message: 'This is the Project Vera API server. Visit https://vera.moeru.ai to use the product, or see the docs at https://vera.moeru.ai/docs.',
      docs: 'https://vera.moeru.ai/docs',
      ui: 'https://vera.moeru.ai',
    }))
    ...
    .notFound(c => c.json({
      error: 'NOT_FOUND',
      message: `No route matched ${c.req.method} ${new URL(c.req.url).pathname}. This is the vera-api server; the product UI lives at https://vera.moeru.ai.`,
      ui: 'https://vera.moeru.ai',
    }, 404))
