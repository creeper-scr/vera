...
export const useBackgroundStore = defineStore('background-entries', () => {
  ...
  const { data: syncSignal, post: broadcastSync } = useBroadcastChannel({ name: 'vera:background-sync' })
  ...
  const activeBackgroundUrl = computed(() => {
    ...
    if (!veraCardStore.activeCard)
      return null
    const bgId = veraCardStore.activeCard.extensions?.vera?.modules?.activeBackgroundId
    if (!bgId || bgId === 'none') {
      return null
    }
  ...
  const availableBackgrounds = computed(() => {
    const veraCardStore = useVeraCardStore()
    return getCharacterBackgrounds.value(veraCardStore.activeCardId)
  })
  ...
  const journalEntries = computed(() => {
    const veraCardStore = useVeraCardStore()
    return getCharacterJournalEntries.value(veraCardStore.activeCardId)
  })
  ...
  async function addBackground(
    ...
    const resolvedCharacterId = characterId !== undefined
      ? characterId
      : ((type === 'journal' || type === 'selfie') ? veraCardStore.activeCardId : null)
