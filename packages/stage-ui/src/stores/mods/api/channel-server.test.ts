...
vi.mock('@proj-vera/stage-shared', () => ({
...
describe('channel-server store reconnect', () => {
  ...
  // Regression coverage for https://github.com/moeru-ai/vera/issues/1545
  it('issue #1545: restores connected state and flushes queued sends when the client reports ready after a reconnect', async () => {
    ...
    expect(client.options.possibleEvents).toContain('extension:module:de-announced')
  ...
  it('defers connection for listeners owned by an explicitly initialized route', async () => {
    const store = useModsServerChannelStore()
    const listener = vi.fn()

    store.onEvent('game:coop:capabilities', listener, { autoInitialize: false })

    expect(serverSdkMocks.MockClient.instances).toHaveLength(0)

    const initializePromise = store.initialize({ token: 'companion-token' })
    const client = serverSdkMocks.MockClient.instances[0]

    expect(client.options.token).toBe('companion-token')
    expect(client.options.possibleEvents).toEqual(expect.arrayContaining([
    ...
    client.simulateAuthenticated()
    await initializePromise
    client.emit('game:coop:capabilities', { capabilities: [] })
