 ...
 * entities, players, etc. Raised from the original 16 to 48 so Vera can find/act on things farther
...
/**
 * Keeps registry-known block ids. Alias expansions may include version-newer
 * names; skip those instead of failing the whole collect scan.
 *
 * A single unknown name still throws with a closest-name suggestion.
 */
function resolveKnownBlockNames(mcData: McData, names: string[]): string[] {
  const known: string[] = []
  const unknown: string[] = []
  for (const name of names) {
    if (mcData.getBlockId(name))
      known.push(name)
    else
      unknown.push(name)
  }
  ...
  const first = unknown[0] ?? names[0] ?? ''
  const closest = mcData.getClosestBlockName(first)
  const suggestion = closest ? `; did you mean ${closest}?` : ''
  throw new Error(`Unknown block type: ${first}${suggestion}`)
}
...
export function getNearestBlocks(mineflayer: Mineflayer, blockTypes: string[] | string | null = null, distance: number = DEFAULT_SCAN_RADIUS, count: number = 10000): Block[] {
  ...
  const blockNames = blockTypes === null
    ...
    : resolveKnownBlockNames(mcData, Array.isArray(blockTypes) ? blockTypes : [blockTypes])
