...
function createVeraCommandEvent() {
  return {
    type: 'perception',
    payload: {
      type: 'vera_command',
      description: 'Directive from Vera: "continue"',
      sourceId: 'vera',
      confidence: 1,
      timestamp: Date.now(),
      metadata: { message: 'continue', sparkCommandId: 'spark-1', sparkIntent: 'action' },
    },
    source: { type: 'vera', id: 'vera' },
    timestamp: Date.now(),
  } as any
}
...
`))
  ...
  it('clears giveUp and proceeds when a Vera command arrives', async () => {
    const deps: any = createDeps('await skip()')
    const brain: any = new Brain(deps)
    brain.givenUp = true
    brain.giveUpReason = 'stuck'
    ...
    await brain.processEvent({} as any, createVeraCommandEvent())
  ...
  it('keeps suppressing non-chat and non-Vera perceptions while giveUp is active', async () => {
    const deps: any = createDeps('await skip()')
    const brain: any = new Brain(deps)
    brain.givenUp = true
    brain.giveUpReason = 'stuck'
...
describe('brain queue coalescing', () => {
  ...
  it('promotes Vera commands ahead of queued ordinary perceptions', () => {
    const brain: any = new Brain(createDeps('await skip()'))

    brain.queue = [
      { event: createNonResumingPerceptionEvent(), resolve: vi.fn(), reject: vi.fn() },
      { event: createFeedbackEvent(), resolve: vi.fn(), reject: vi.fn() },
      { event: createVeraCommandEvent(), resolve: vi.fn(), reject: vi.fn() },
    ]

    brain.coalesceQueue()

    expect(brain.queue[0].event.type).toBe('perception')
    expect((brain.queue[0].event.payload as any).type).toBe('vera_command')
