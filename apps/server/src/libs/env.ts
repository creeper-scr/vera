...
const EnvSchema = object({
  ...
  // of the server image.
  AUTH_UI_URL: optional(string(), 'https://accounts.vera.build/ui'),
  ...
  // the standalone proj-vera repository.
  ADMIN_UI_URL: optional(string(), 'https://admin.vera.build'),
  ...
  WEB_APP_URL: optional(string(), 'https://vera.moeru.ai'),
  ...
  // Social sign-in is optional for local/core-runtime deployments. A provider
  // is only registered when both values are configured.
  ...
  RESEND_FROM_EMAIL: optional(string(), 'noreply@vera.moeru.ai'),
  // Optional friendly name; rendered as `Name <email>` per Resend's RFC 5322 display-name format.
  RESEND_FROM_NAME: optional(string(), 'Project Vera'),
  ...
  // Defaults to the shared Vera project key (same browser-safe phc_* key the
  ...
  POSTHOG_API_HOST: optional(string(), 'https://t.vera.build'),

  // OpenTelemetry
  OTEL_SERVICE_NAMESPACE: optional(string(), 'vera'),
