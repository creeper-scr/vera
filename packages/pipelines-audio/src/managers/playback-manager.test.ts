...
describe('createPlaybackManager', () => {
  it.each(['stopByIntent', 'stopByOwner', 'stopAll'] as const)(
    ...
    (method) => {
      ...
      manager.schedule(createPlaybackItem('active', 10, 'intent-1', 'owner-1'))
      manager.schedule(createPlaybackItem('queued', 5, 'intent-2', 'owner-2'))

      if (method === 'stopByIntent')
        manager.stopByIntent('intent-1', 'stop')
      else if (method === 'stopByOwner')
        manager.stopByOwner('owner-1', 'stop')
      else
  ...
  it('fills remaining capacity after stealing an owner-overflow playback slot', async () => {
    ...
    expect(play).toHaveBeenNthCalledWith(4, expect.objectContaining({ id: 'b' }), expect.any(AbortSignal))
  })

  it('does not cascade owner-overflow steals during one queue wake', async () => {
    const resolveMap = new Map<string, () => void>()
    const play = vi.fn((item: PlaybackItem<unknown>, signal) => new Promise<void>((resolve) => {
      resolveMap.set(item.id, resolve)
      signal.addEventListener('abort', resolve, { once: true })
    ...
    manager.schedule(createPlaybackItem('a', 10, 'intent-1', 'owner-x'))
    manager.schedule(createPlaybackItem('d', 10, 'intent-2', 'owner-y'))
    manager.schedule(createPlaybackItem('e', 10, 'intent-3', 'owner-z'))
    manager.schedule(createPlaybackItem('a2', 9, 'intent-4', 'owner-x'))
    manager.schedule(createPlaybackItem('d2', 8, 'intent-5', 'owner-y'))
    manager.schedule(createPlaybackItem('f', 7, 'intent-6', 'owner-w'))
    ...
    expect(play).toHaveBeenNthCalledWith(4, expect.objectContaining({ id: 'a2' }), expect.any(AbortSignal))
    expect(play).toHaveBeenNthCalledWith(5, expect.objectContaining({ id: 'f' }), expect.any(AbortSignal))
  })
})
