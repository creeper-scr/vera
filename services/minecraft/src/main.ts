...
import { Client } from '@proj-vera/server-sdk'
...
import { startVeraClientConnection } from './vera/start-background-client'
...
async function main() {
  ...
  // Connect Vera server
  let veraConnectionLifecycle: ReturnType<typeof startVeraClientConnection> | null = null
  const veraClient = new Client({
    name: config.vera.clientName,
    url: config.vera.wsBaseUrl,
    token: config.vera.token || undefined,
    possibleEvents: [
      'module:configure',
      'module:announced',
      'spark:command',
    ...
    // readTimeout would trip and tear down the Vera connection — making the in-game bot flap
    ...
    onError: error => veraConnectionLifecycle?.reportUnavailable(error),
    onClose: () => veraConnectionLifecycle?.reportDisconnected(),
  })
  veraConnectionLifecycle = startVeraClientConnection(veraClient, {
    logger,
    url: config.vera.wsBaseUrl,
  })
  ...
  async function createManagedBot(botConfig: typeof config.bot) {
    const runtime = new MinecraftBotRuntime({
      ...
      createBot: async (nextBotConfig) => {
        ...
        await bot.loadPlugin(CognitiveEngine({ veraClient }))
  ...
  veraClient.onEvent('module:configure', async (event) => {
    try {
  ...
  process.on('SIGINT', () => {
    Promise.resolve(activeRuntime?.stop())
      ...
      .finally(() => {
        // TODO: Add an explicit Vera-side deregistration path on shutdown instead of relying on
