...
for the upcoming memory update for Project AIRI (upstream). Which we were planning to implement
for months.
...
We also added Character Card support to Project AIRI (upstream). Character Cards are JSON files that define
...
```json
{
  ...
  "creator": "Project AIRI (upstream) Team",
...
We've been working on improving the visual theme of Project AIRI (upstream). The new theme system now supports:
...
We're excited to see the community starting to contribute to Project AIRI (upstream)! Some notable contributions include:
...
We're also planning to release more detailed documentation about each component of Project AIRI (upstream),
including architecture deep dives and implementation guides.

## Conclusion

It's been an exciting period of development for Project AIRI (upstream). The memory system is taking shape,
...
— The Project AIRI (upstream) Team
