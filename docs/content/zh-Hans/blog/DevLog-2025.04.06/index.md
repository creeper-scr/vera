...
最近正在重构 [`telegram-bot`](https://github.com/moeru-ai/airi/tree/main/services/telegram-bot) 以为已经准备了数月的 Project AIRI (upstream) 即将到来的「记忆更新」作准备。
...
- `@proj-vera/drizzle-duckdb-wasm`, `@proj-vera/duckdb-wasm`
  用于为 Drizzle 添加 DuckDB WASM 驱动支持的 `@proj-vera/drizzle-duckdb-wasm` 和 `@proj-vera/duckdb-wasm` 也算是阶段性毕业了，现在正式迁移到 [@proj-airi](https://github.com/proj-airi) 的组织名下（不需要任何迁移操作，继续安装原来的包就可以用了）。

现在项目速度快了很多，这个月应该会把 `@proj-vera/providers-transformers` 正式毕业到 `xsai` 名下。
...
当然，一个包含模型、声线和 Project AIRI (upstream) 支持的所有模块 🎉 的配置的能力的编辑器也包含在内了。
...
- 为 Project AIRI (upstream) 的特殊 DevLog 新增了 98.css 集成（感谢 [@OverflowCat](https://github.com/OverflowCat)）

> 这是 Project AIRI (upstream) 一篇特别版的开发日志，其灵感主要来自 [@OverflowCat](https://github.com/OverflowCat) 的博文 [ModTran](https://blog.xinshijiededa.men/modtran/)，代码风格大量借鉴了 [@OverflowCat](https://github.com/OverflowCat) 在 https://github.com/OverflowCat/blog/blob/0a92f916629ad942b7da84b894759fde1616bf37/src/components/98/98.ts 里的实现。
>
