...
import type { WebSocketEventOf } from '@proj-vera/server-sdk'
...
import type { VeraCard } from '../../modules'
...
import { useConsciousnessStore, useVeraCardStore } from '../../modules'
...
describe('store character-orchestrator', () => {
  beforeEach(() => {
    ...
    const veraCardStore = useVeraCardStore(pinia)
    // @ts-expect-error - testing purpose
    veraCardStore.systemPrompt = 'You are a brave adventurer in Minecraft.'
    // @ts-expect-error - testing purpose
    veraCardStore.activeCard = {
      name: 'Hero',
      version: '1.0',
      extensions: {
        vera: {
  ...
  it('supports forcing text-only spark:notify responses', async () => {
    ...
    const event: WebSocketEventOf<'spark:notify'> = {
      type: 'spark:notify',
      source: 'plugin:vera-plugin-game-chess',
      data: {
        id: nanoid(),
        eventId: nanoid(),
        kind: 'ping',
        urgency: 'immediate',
        headline: 'Vera played d5',
        destinations: ['character'],
      },
    }
