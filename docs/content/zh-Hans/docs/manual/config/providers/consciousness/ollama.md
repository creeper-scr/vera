---
title: Ollama（本地模型）
description: 使用本地 Ollama 服务为 Vera 配置聊天模型
---
...
1. **Ping API**：点击此按钮测试 Vera 能否连接本地服务。
2. **选择模型**：测试成功后，点击此处选择已下载的模型。

## 排查

无法连接时，先确认 Ollama 正在运行，且端口与 Base URL 一致。若 Vera 与 Ollama 不在同一设备，使用可从 Vera 设备访问的局域网地址，并仅在可信网络中开放该服务。

::: warning Vera 无法连接本地 Ollama
如果 Ollama 正在运行但 Vera 显示网络或 CORS 错误，请按 Ollama 的启动方式设置 `OLLAMA_ORIGINS`，允许 Vera 的来源访问服务后再重启 Ollama。不要为了排查问题而将本地服务直接暴露到公网。
:::
