...
describe('bug report payload helpers', () => {
  it('builds markdown payload with description and optional context/screenshot flags', () => {
    const payload = buildBugReportPayload({
      ...
      context: {
        url: 'https://vera.local/chat?room=debug',
        title: 'Vera Chat',
    ...
    expect(payload).toContain('- URL: https://vera.local/chat?room=debug')
  ...
  it('extracts page context from a window-like object', () => {
    const context = createBugReportPageContext({
      location: {
        href: 'https://vera.local/settings?tab=providers',
      },
      document: {
        title: 'Settings',
      },
    ...
    expect(context).toEqual({
      url: 'https://vera.local/settings?tab=providers',
      title: 'Settings',
      userAgent: 'unit-test',
      viewport: '1280x720',
      language: 'en-US',
