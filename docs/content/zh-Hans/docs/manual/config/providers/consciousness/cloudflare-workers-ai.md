...
description: 在 Vera 中配置 Cloudflare Workers AI 聊天模型
---

Cloudflare Workers AI 使用账户级凭据。除 API Token 外，Vera 还需要 Cloudflare Account ID 才能定位你的 Workers AI 资源。
...
::: warning 安全提醒
API Token 与账户权限绑定。请遵循最小权限原则，只授予 Vera 所需的 Workers AI 权限；不要将 Token 或 Account ID 与公开日志一同发布。
:::
...
若 Vera 提示凭据无效，分别检查 Token 权限和 Account ID 是否来自同一个 Cloudflare 账户。此服务商不使用可编辑的 Base URL，因此不应将 Worker URL 或 API 路径填入任何字段。
