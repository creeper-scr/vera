...
/**
 * Wire the `vera.gen_ai.gateway.pool.inflight` ObservableGauge to the Redis-backed
 ...
 *
 * Failure mode:
 * - On Redis error we increment `vera.observability.read_errors{metric}` and
...
export function registerTtsPoolGauge(
  ...
  async function refresh(): Promise<boolean> {
    try {
      ...
      metricReadErrors.add(1, { metric: 'vera.gen_ai.gateway.pool.inflight' })
      return false
    }
  }
