...
export interface SpeechIntentTerminalPayload {
  /** Runtime that opened the intent and must receive this terminal result. */
  originId: string
  intentId: string
  result:
    | { status: 'completed' }
...
export const speechIntentTerminalEvent = defineEventa<SpeechIntentTerminalPayload>('eventa:audio:speech:intent:terminal')

const BUS_CHANNEL_NAME = 'proj-vera:pipelines:outputs:speech'
