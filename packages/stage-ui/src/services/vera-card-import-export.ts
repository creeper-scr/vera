...
const FORMAT = 'vera-character-card'
const VERSION = 1
...
type ExportableCard = Card & { extensions: { vera: VeraExtension } }
...
type VeraCardPackageErrorCode = 'missing-file' | 'invalid-file'
type Manifest = InferOutput<typeof manifestSchema>

export class VeraCardPackageError extends Error {
  constructor(public readonly code: VeraCardPackageErrorCode, message: string, options?: { cause?: unknown }) {
    super(message, options)
    this.name = 'VeraCardPackageError'
  }
}

/** Exports only the creation/edit form whitelist; provider globals and runtime state are never cloned. */
export async function exportVeraCardPackage({ card, displayModelsStore }: { card: VeraCard, displayModelsStore: DisplayModelsStore }): Promise<Blob> {
  const exportableCard = cardFromVeraCard(card)
...
/** Imports a package as sanitized CCv3 JSON; edited zip payloads cannot smuggle extra Vera fields through. */
export async function importVeraCardPackage({ file, displayModelsStore }: { file: File, displayModelsStore: DisplayModelsStore }): Promise<ccv3.CharacterCardV3> {
...
async function exportDisplayModel(card: ExportableCard, store: DisplayModelsStore) {
  const displayModelId = card.extensions.vera.modules.displayModelId
  if (!displayModelId)
    return
...
function cardFromVeraCard(card: VeraCard): ExportableCard {
  return {
    name: card.name,
    ...
    extensions: { vera: sanitizeVera(card.extensions?.vera) },
  }
}
...
function cardFromCharacterCard(card: CharacterCardPackageJson, displayModelId?: string): ExportableCard {
  ...
  return {
    ...
    extensions: { vera: sanitizeVera(data.extensions?.vera, displayModelId) },
  }
}

function sanitizeVera(value: unknown, displayModelIdOverride?: string): VeraExtension {
...
function isSpawnMode(value: unknown): value is NonNullable<VeraExtension['modules']['artistry']>['spawnMode'] {
...
function error(code: VeraCardPackageErrorCode, message: string, options?: { cause?: unknown }) {
  return new VeraCardPackageError(code, message, options)
}
