...
import { defaultMobileHtmlPath, lanSharePlugin } from './src/server/lan-share-plugin'
...
/**
 * Load play env into `process.env` (do not override existing).
 * Order: `.env.play.local` then `services/minecraft/.env.local`.
 */
function applyPlayEnvFile() {
  const root = resolve(join(import.meta.dirname, '..', '..'))
  const files = [
    join(root, '.env.play.local'),
    join(root, 'services', 'minecraft', '.env.local'),
  ]
...
/**
 * Map play-file `OPENAI_*` onto `import.meta.env.VITE_OPENAI_*` so Web Companion
 * can seed consciousness from the same credentials as minecraft-bot.
 */
function playLlmEnvDefine() {
  const apiKey = env.VITE_OPENAI_API_KEY || env.OPENAI_API_KEY || ''
  const baseUrl = env.VITE_OPENAI_API_BASEURL || env.OPENAI_API_BASEURL || ''
  const model = env.VITE_OPENAI_MODEL || env.OPENAI_MODEL || ''
  const doubaoWs = env.VITE_DOUBAO_REALTIME_WS_URL
    || `ws://127.0.0.1:${env.DOUBAO_REALTIME_WS_PORT || '6122'}/api/v1/audio/realtime/ws`
  return {
    'import.meta.env.VITE_OPENAI_API_KEY': JSON.stringify(apiKey),
    'import.meta.env.VITE_OPENAI_API_BASEURL': JSON.stringify(baseUrl),
    'import.meta.env.VITE_OPENAI_MODEL': JSON.stringify(model),
    'import.meta.env.VITE_DOUBAO_REALTIME_WS_URL': JSON.stringify(doubaoWs),
  }
}

export default defineConfig({
  define: playLlmEnvDefine(),
  optimizeDeps: {
    exclude: [
      // Internal Packages
      '@proj-vera/stage-ui/*',
  ...
  resolve: {
    alias: {
      '@proj-vera/server-sdk': resolve(join(import.meta.dirname, '..', '..', 'packages', 'server-sdk', 'src')),
      '@proj-vera/i18n': resolve(join(import.meta.dirname, '..', '..', 'packages', 'i18n', 'src')),
      '@proj-vera/stage-ui': resolve(join(import.meta.dirname, '..', '..', 'packages', 'stage-ui', 'src')),
      '@proj-vera/stage-pages': resolve(join(import.meta.dirname, '..', '..', 'packages', 'stage-pages', 'src')),
      '@proj-vera/stage-shared': resolve(join(import.meta.dirname, '..', '..', 'packages', 'stage-shared', 'src')),
      '@proj-vera/stage-layouts': resolve(join(import.meta.dirname, '..', '..', 'packages', 'stage-layouts', 'src')),
    },
  },
  server: {
    fs: {
    ...
    /**
     * Same-origin Doubao relay so phone pages on HTTPS can use `wss://` without
     * mixed-content blocks (upstream stays on local :6122).
     */
    proxy: {
      '/api/v1/audio/realtime/ws': {
        target: `ws://127.0.0.1:${env.DOUBAO_REALTIME_WS_PORT || '6122'}`,
        changeOrigin: true,
        ws: true,
      },
    },
  ...
  plugins: [
    ...
    lanSharePlugin({
      mobileHtmlPath: defaultMobileHtmlPath(import.meta.dirname),
    }),
    ...
    VueRouter({
      ...
      // No hosted auth / account / billing surfaces in the web companion fork.
      exclude: [
        '**/components/**',
        '**/auth/**',
        '**/settings/account/**',
    ...
    ...((!env.S3_ENDPOINT || !env.S3_ACCESS_KEY_ID || !env.S3_SECRET_ACCESS_KEY)
      ...
      : [
          Basemove({
            prefix: env.STAGE_WEB_WARP_DRIVE_PREFIX || 'proj-vera/stage-web/main/',
