...
import { artistryGenerateHeadless } from '@proj-vera/stage-shared'
...
import { useVeraCardStore } from './vera-card'
...
export const useAutonomousArtistryStore = defineStore('artistry-autonomous', () => {
  ...
  async function runArtistTask(inputText: string, history: Message[] = [], targetOverride?: 'user' | 'assistant') {
    ...
    const artistry = activeCard?.extensions?.vera?.modules?.artistry
    ...
    try {
      ...
      const systemPrompt = target === 'assistant'
        ? `You are the Cinematic Director for Vera. 
            ...
            case 'bg':
              ...
              cardStore.updateCard(cardId, {
                extensions: {
                  ...
                  vera: {
                    ...activeCard.extensions.vera,
                    modules: {
                      ...activeCard.extensions.vera.modules,
