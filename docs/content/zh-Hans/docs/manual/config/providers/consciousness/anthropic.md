---
title: Anthropic
description: 在 Vera 中配置 Anthropic Claude 聊天模型
---

Anthropic 服务商让 Vera 使用 Claude 聊天模型。Vera 使用 Anthropic 的 API 地址和你的 API Key；模型列表由 Vera 内置，因此无需手动填写 Base URL 或模型 ID 才能开始。

::: info 为什么选择 Anthropic？
如果你已在使用 Claude API，或希望在 Vera 中使用 Claude 模型，可以直接选择 Anthropic。
:::
...
若模型选择器没有显示预期模型，先更新 Vera 或在“意识”页面手动输入服务商提供的精确模型 ID。
