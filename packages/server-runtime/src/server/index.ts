import type { H3CrossWsApp, H3CrossWsResponse } from '@proj-vera/better-ws/server/h3'
...
import { createH3CrossWsPlugin } from '@proj-vera/better-ws/server/h3'
...
/**
 * Creates the websocket server controller for the Vera runtime.
 *
 * Use when:
...
export function createServer(opts?: ServerOptions): Server {
  ...
  const log = useLogg('@proj-vera/server-runtime/server').withLogLevelString(appLogLevel).withFormat(appLogFormat)
    ...
    startTask = (async () => {
      ...
      try {
        ...
        if (hostname === '0.0.0.0') {
          ...
          log.log(`@proj-vera/server-runtime started on ${protocol}://0.0.0.0:${port} (reachable via: ${targets})`)
        }
        else {
          log.log(`@proj-vera/server-runtime started on ${protocol}://${hostname}:${port}`)
        }
      }
      catch (error) {
        serverInstance = null
