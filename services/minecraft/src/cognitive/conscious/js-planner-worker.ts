...
process.on('disconnect', () => {
  ...
  pendingRequests.clear()
  // Parent is gone. Exit even if no pending bridge calls remain.
  process.exit(1)
})
...
// Parent-liveness watchdog in a separate worker thread.
//
// process.on('disconnect') only runs when the main event loop is free. User
// code that blocks the isolate/host loop (infinite loop, tight spin) leaves
// the worker as a CPU-spinning orphan after the parent dies, because disconnect
// never fires. A worker_threads interval keeps checking process.ppid even when
// the main thread is wedged.
//
// NOTICE:
// postMessage → process.exit on the main thread also fails when the main loop
// is wedged (the message sits in the queue forever). The watchdog therefore
// SIGKILLs the worker process itself from the side thread so exit does not
// depend on main-thread scheduling.
...
const parentWatchdog = new WatchdogWorker(`
  ...
  setInterval(() => {
    try {
      ...
      // ESRCH = no such process. EPERM means the process still exists but we
      // cannot signal it — do not treat that as parent death.
