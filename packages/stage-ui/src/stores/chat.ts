import type { ChatOrchestratorRuntimeState, ChatOrchestratorSendOptions, StreamEvent, StreamOptions } from '@proj-vera/core-agent'
import type { ChatProvider } from '@xsai-ext/providers/utils'
import type { Message } from '@xsai/shared-chat'

import type { ChatHistoryItem } from '../types/chat'

import { createChatOrchestratorRuntime } from '@proj-vera/core-agent'
import { IOAttributes, IOEvents, IOSpanNames, IOSubsystems } from '@proj-vera/stage-shared'
...
import {
  VERA_CHAT_APP_SURFACE_HEADER,
  VERA_CHAT_ROUND_ID_HEADER,
  VERA_CHAT_SESSION_ID_HEADER,
...
import { useVeraCardStore } from './modules/vera-card'
...
export type { QueuedSendSnapshot, ChatOrchestratorSendOptions as SendOptions } from '@proj-vera/core-agent'
...
export const useChatOrchestratorStore = defineStore('chat-orchestrator', () => {
  ...
  async function streamWithStageAdapters(
    ...
    if (providerMode(activeProvider.value) === 'official' && options?.requestCorrelation) {
      headers[VERA_CHAT_SESSION_ID_HEADER] = options.requestCorrelation.conversationId
      headers[VERA_CHAT_ROUND_ID_HEADER] = options.requestCorrelation.roundId
      headers[VERA_CHAT_APP_SURFACE_HEADER] = getConversationAnalyticsSurface()
    }
  ...
  const runtime = createChatOrchestratorRuntime({
    ...
    onUserTurnReady: ({ messageText, sessionMessages }) => {
      const autonomousTarget = cardStore.activeCard?.extensions?.vera?.modules?.artistry?.autonomousTarget || 'user'
      if (autonomousTarget === 'user')
        void artistryAutonomousStore.runArtistTask(messageText, toProviderHistory(sessionMessages))
    },
    onAssistantTurnReady: ({ messageText, sessionMessages }) => {
      const artistry = cardStore.activeCard?.extensions?.vera?.modules?.artistry
