...
// For MToon IBL
let veraIblProbe: ReturnType<typeof createIblProbeController> | null = null
  ...
  try {
    ...
    if (reusableInstance) {
      if (!isManagedVrmInstanceReusable(reusableInstance)) {
        ...
        if (!veraIblProbe && scene.value)
          veraIblProbe = createIblProbeController(scene.value)
...
onMounted(async () => {
  ...
  */
  ...
  watch([envSelect, skyBoxIntensity, nprIrrSH], async () => {
    ...
    veraIblProbe?.update(mode, skyBoxIntensity.value, nprIrrSH.value ?? null)
  }, { immediate: true })
