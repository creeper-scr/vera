...
describe('origin utils', () => {
  ...
  it('allows the standalone auth and admin UI origins', () => {
    expect(getTrustedOrigin('https://accounts.vera.build')).toBe('https://accounts.vera.build')
    expect(getTrustedOrigin('https://server-dev.vera-server-auth.pages.dev')).toBe('https://server-dev.vera-server-auth.pages.dev')
    expect(getTrustedOrigin('https://admin.vera.build')).toBe('https://admin.vera.build')
    expect(getTrustedOrigin('https://server-dev.vera-server-admin.pages.dev')).toBe('https://server-dev.vera-server-admin.pages.dev')
  })
  ...
  it('prefers a trusted referer origin', () => {
    const request = new Request('http://localhost/api/v1/stripe/checkout', {
      headers: {
        referer: 'https://vera.moeru.ai/settings/flux',
        origin: 'https://example.com',
      },
    })

    expect(resolveTrustedRequestOrigin(request)).toBe('https://vera.moeru.ai')
  })
  ...
  it('collects api and request origins for auth', () => {
    ...
    expect(getAuthTrustedOrigins({
      API_SERVER_URL: 'https://api.vera.moeru.ai',
      ADDITIONAL_TRUSTED_ORIGINS: [],
    }, request)).toEqual([
      'https://api.vera.moeru.ai',
      'https://vera.moeru.ai',
      'https://accounts.vera.build',
      'https://server-dev.vera-server-auth.pages.dev',
      'https://admin.vera.build',
      'https://server-dev.vera-server-admin.pages.dev',
      'http://localhost:*',
      'http://127.0.0.1:*',
      'http://localhost:5173',
    ])
  ...
  describe('resolveCheckoutRedirectBase', () => {
    const fallback = 'https://vera.moeru.ai'
  ...
  it('does not include native deep-link schemes in Better Auth trustedOrigins', () => {
    ...
    expect(getTrustedOrigin('ai.moeru.vera-pocket://links')).toBe('ai.moeru.vera-pocket://links')

    const authOrigins = getAuthTrustedOrigins({
      API_SERVER_URL: 'https://api.vera.build',
      ADDITIONAL_TRUSTED_ORIGINS: [],
    })

    expect(authOrigins).not.toContain('capacitor://localhost')
    expect(authOrigins).not.toContain('ai.moeru.vera-pocket://links')
  })
  ...
  // UI sends callbackURL=https://accounts.vera.build/ui/verify-email?verified=true,
  ...
  it('includes built-in first-party origins for email verification callbacks without request headers', () => {
    expect(getAuthTrustedOrigins({
      API_SERVER_URL: 'https://api.vera.build',
      ADDITIONAL_TRUSTED_ORIGINS: [],
    })).toContain('https://accounts.vera.build')
  })
})
