...
import type { Live2DLipSync, Live2DLipSyncOptions } from '@proj-vera/model-driver-lipsync'
import type { Profile } from '@proj-vera/model-driver-lipsync/shared/wlipsync'
import type { VrmInteractionTarget } from '@proj-vera/stage-ui-three'
...
import { createLive2DLipSync } from '@proj-vera/model-driver-lipsync'
import { wlipsyncProfile } from '@proj-vera/model-driver-lipsync/shared/wlipsync'
import { createPlaybackManager, createSpeechPipeline, normalizeActPayload } from '@proj-vera/pipelines-audio'
import { Live2DScene, useLive2dParams } from '@proj-vera/stage-ui-live2d'
import { MMDScene } from '@proj-vera/stage-ui-mmd'
import { SpineScene } from '@proj-vera/stage-ui-spine'
import { ThreeScene } from '@proj-vera/stage-ui-three'
import { animations } from '@proj-vera/stage-ui-three/assets/vrm'
import { createQueue } from '@proj-vera/stream-kit'
import { Callout } from '@proj-vera/ui'
...
const { post: postCaption } = useBroadcastChannel<CaptionChannelEvent, CaptionChannelEvent>({ name: 'vera-caption-overlay' })
const assistantCaption = ref('')

type PresentEvent
  = | { type: 'assistant-reset' }
    | { type: 'assistant-append', text: string }
const { post: postPresent } = useBroadcastChannel<PresentEvent, PresentEvent>({ name: 'vera-chat-present' })
...
const { activeCard } = storeToRefs(useVeraCardStore())
