...
describe('useLinkedAccounts', () => {
  it('passes the profile page URL as the OAuth link error callback URL', async () => {
    ...
    const app = createSSRApp({
      setup() {
        holder.linkedAccounts = useLinkedAccounts({
          ...
          describeError: () => '',
          buildCallbackURL: () => 'https://accounts.vera.build/ui/profile',
          messages: {
    ...
    expect(linkSocial).toHaveBeenCalledWith({
      provider: 'github',
      callbackURL: 'https://accounts.vera.build/ui/profile',
      errorCallbackURL: 'https://accounts.vera.build/ui/profile',
    })
  })
