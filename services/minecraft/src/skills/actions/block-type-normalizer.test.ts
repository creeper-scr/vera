...
describe('block-type-normalizer', () => {
  ...
  it('expands wood aliases to concrete log and stem ids', () => {
    for (const alias of ['wood', 'tree', 'log', 'logs', '木头', '木材', '原木', '树']) {
      const aliases = expandCollectibleBlockAliases(alias)
      expect(aliases).toContain('oak_log')
      expect(aliases).toContain('spruce_log')
      expect(aliases).toContain('birch_log')
      expect(aliases).toContain('cherry_log')
    }
  })
