...
export async function executeSandboxWorker(
  ...
  return await new Promise<SandboxWorkerResult>((resolve, reject) => {
    ...
    const child = fork(SANDBOX_WORKER_ENTRY_PATH, [ISOLATED_VM_ENTRY_PATH], {
      ...
      execArgv: [
        '--permission',
        // Parent-liveness watchdog in js-planner-worker uses worker_threads so it
        // still runs when user code blocks the main event loop. Without this flag,
        // Worker construction throws ERR_ACCESS_DENIED under --permission.
        '--allow-worker',
        '--allow-addons',
        `--allow-fs-read=${SANDBOX_SOURCE_DIRECTORY}`,
        `--allow-fs-read=${PNPM_MODULE_STORE_PATH}`,
        // NOTICE: node-gyp-build (via isolated-vm) calls existsSync('/etc/alpine-release')
        // during native addon load. Under --permission that probe throws ERR_ACCESS_DENIED
        // unless /etc is readable, which aborts every sandbox evaluation before user code runs.
        '--allow-fs-read=/etc',
        '--disable-proto=throw',
        '--frozen-intrinsics',
