import type { ContextUpdate, ModuleAnnouncedEvent } from '@proj-vera/server-sdk'

import type { MineflayerWithAgents } from '../cognitive/types'
import type { VeraBridge } from './vera-bridge'
...
export class MinecraftContextService {
  ...
  constructor(private readonly deps: {
    veraBridge: Pick<VeraBridge, 'onModuleAnnounced' | 'sendContextUpdate'>
    serverHost: string
    serverPort: number
    masterUsername?: string
  ...
  init() {
    ...
    this.unsubscribeModuleAnnounced = this.deps.veraBridge.onModuleAnnounced((event) => {
  ...
  publishStatus(options: { force?: boolean, destinations?: string[] } = {}) {
    ...
    this.deps.veraBridge.sendContextUpdate(update)
