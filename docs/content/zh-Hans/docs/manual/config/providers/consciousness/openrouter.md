---
title: OpenRouter
description: 在 Vera 中配置 OpenRouter 作为大模型服务商
is_openai_compatible: true
---

OpenRouter 是一个聚合式 API 服务商。完成本页配置后，Vera 可以在“意识”中使用 OpenRouter 提供的聊天模型。

::: info 为什么选择 OpenRouter？
如果你希望用一个 API Key 在 Vera 中尝试多个模型，OpenRouter 是一个方便的选择。它将多个模型服务集中到同一套接口和账单中，因此切换模型时通常不必分别配置多个服务商。在中国大陆使用 Vera 时，可以优先尝试 302.AI；实际可用性仍取决于你的网络环境、支付方式和服务商政策。
:::
