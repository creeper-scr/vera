...
import { Client, createTextProtocolConnector, WebSocketEventSource } from '@proj-vera/server-sdk'
import { isStageTamagotchi, isStageWeb } from '@proj-vera/stage-shared'
...
export const useModsServerChannelStore = defineStore('mods:channels:proj-vera:server', () => {
  ...
  // Protocol events form a large discriminated union. Payloads are immutable
  // transport records, so Vue must not recursively unwrap them.
  const pendingSend = shallowRef<Array<WebSocketEvent>>([])
  const pendingSendCount = computed(() => pendingSend.value.length)
  const reconnectedCallbacks = new Set<() => void>()

  const defaultWebSocketUrl = import.meta.env.VITE_VERA_WS_URL || 'ws://localhost:6121/ws'
  ...
  function registerListener<E extends keyof WebSocketEvents>(
    ...
    if (options.autoInitialize !== false && !client.value && !initializing.value)
      void initialize()
  ...
  function onEvent<E extends keyof WebSocketEvents>(
    ...
    return registerListener(type, callback, options)
  }
