---
title: X / Twitter
description: 使用 X Developer Platform 凭据启用 Vera 的 X / Twitter 集成
---

X / Twitter 集成使用 X Developer Platform 应用的四项凭据，让 Vera 的服务通道能够请求 X / Twitter 功能。凭据由你自己的 X 开发者账号签发；是否具备读取、发布或其他能力取决于该应用已获授的权限和 X 的套餐限制。
...
::: warning 凭据安全
API Key、API Secret、Access Token 与 Access Token Secret 等同于应用的访问凭据。只在 Vera 的本机设置中填写它们；不要提交到仓库、放进截图、贴到 Issue 或发送给他人。怀疑泄露时，请立即在 X Developer Portal 中重新生成对应凭据。
:::
...
“已配置”表示 Vera 已保存凭据，并不保证所有请求都能成功；X 会根据应用权限、账号状态、访问套餐和速率限制决定实际结果。
