...
describe('useAnalytics conversation product events', () => {
  ...
  it('emits Vera card edit and scene background events', () => {
    const analytics = useAnalytics()
  ...
  it('emits desktop differentiator events for spotlight, widgets, updater, MCP, and pverang', () => {
    analyticsMocks.isStageTamagotchiMock.mockReturnValue(true)
    const analytics = useAnalytics()
    ...
    analytics.trackDevicePverangQrShown()
    ...
    expect(analyticsMocks.posthogCaptureMock).toHaveBeenNthCalledWith(9, 'device_pverang_qr_shown')
  })
})
