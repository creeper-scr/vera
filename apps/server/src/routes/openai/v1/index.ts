...
export function createV1Routes(input: CreateV1RoutesDeps) {
    ...
    .post('/chat/completions', openai.handler(
      ...
      async (c) => {
        ...
        return {
          userId: user.id,
          body,
          sessionId: c.req.header(VERA_CHAT_SESSION_ID_HEADER),
          roundId: c.req.header(VERA_CHAT_ROUND_ID_HEADER),
          appSurface: resolveChatAnalyticsSurface(c.req.header(VERA_CHAT_APP_SURFACE_HEADER)),
  ...
  // Vera audio surface (mounted at /api/v1/audio). Lives outside /openai/ so
