import type { Card, ccv3 } from '@proj-vera/ccc'
...
export interface VeraExtension {
  modules: {
    consciousness: {
...
export interface VeraCard extends Card {
  extensions: {
    vera: VeraExtension
  } & Card['extensions']
}

export const useVeraCardStore = defineStore('vera-card', () => {
  const { t } = useI18n()

  const cards = useLocalStorageManualReset<Map<string, VeraCard>>('vera-cards', new Map())
  const activeCardId = useLocalStorageManualReset<string>('vera-card-active-id', 'default')
   ...
   */
  const addCard = (card: VeraCard | Card | ccv3.CharacterCardV3, source: 'scratch' | 'import' | 'duplicate') => {
    const newCardId = nanoid()
    cards.value.set(newCardId, newVeraCard(card))
  ...
  const updateCard = (id: string, updates: VeraCard | Card | ccv3.CharacterCardV3) => {
    ...
    cards.value.set(id, newVeraCard(updatedCard))
    return true
  }
  ...
  function updateActiveCardModules(patch: (extension: VeraExtension) => Partial<VeraExtension['modules']>) {
    const cardId = activeCardId.value
    const card = cards.value.get(cardId)
    if (!card)
      return false

    const extension = resolveVeraExtension(card)
  ...
  function updateActiveCardConsciousness(consciousness: VeraExtension['modules']['consciousness']) {
    return updateActiveCardModules(() => ({ consciousness }))
  }

  function updateActiveCardVision(vision: VeraExtension['modules']['vision']) {
    return updateActiveCardModules(() => ({ vision }))
  }

  function updateActiveCardSpeech(speech: Pick<VeraExtension['modules']['speech'], 'provider' | 'model' | 'voice_id'>) {
  ...
  function resolveVeraExtension(card: Card | ccv3.CharacterCardV3): VeraExtension {
    // Get existing extension if available
    const existingExtension = ('data' in card
      ? card.data?.extensions?.vera
      : card.extensions?.vera) as VeraExtension
  ...
  function newVeraCard(card: Card | ccv3.CharacterCardV3): VeraCard {
    ...
    if ('data' in card) {
      ...
      return {
        ...
        extensions: {
          vera: resolveVeraExtension(ccv3Card),
    ...
    return {
      ...card,
      extensions: {
        vera: resolveVeraExtension(card),
        ...card.extensions,
      },
    }
  }
  ...
  function initialize() {
    if (cards.value.has('default'))
      return
    cards.value.set('default', newVeraCard({
      name: 'ReLU',
      version: '1.0.0',
  ...
  watchDebounced(activeCard, (newCard: VeraCard | undefined) => {
    ...
    const extension = resolveVeraExtension(newCard)
    if (!extension)
      return
  ...
  return {
    ...
    currentModels: computed(() => {
      return {
        ...
        displayModelId: stageModelStore.stageModelSelected,
        activeBackgroundId: activeCard.value?.extensions?.vera?.modules?.activeBackgroundId,
      } satisfies VeraExtension['modules']
    }),
    ...
    systemPrompt: computed(() => {
      ...
      const components = [
        ...
        card.description,
        card.personality,
        card.extensions?.vera?.modules?.artistry?.widgetInstruction,
      ].filter(Boolean)
