...
description: 在 Vera 网页版中使用浏览器内置语音识别
---
...
1. 使用 Vera 网页版；Web Speech API 不会在桌面版（Electron）中提供。
2. 确认当前浏览器支持 Web Speech API，并准备允许麦克风权限。

::: warning 浏览器限制
Web Speech API 仅适用于浏览器环境，Vera 桌面版（Electron）不支持它。不同浏览器、网络环境和语言的识别能力可能不同。
:::
