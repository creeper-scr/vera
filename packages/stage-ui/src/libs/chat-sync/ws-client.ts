import type { NewMessagesPayload, PullMessagesRequest, PullMessagesResponse, SendMessagesRequest, SendMessagesResponse } from '@proj-vera/server-sdk-shared'
import type { ComputedRef, Ref } from 'vue'

import { defineInvoke } from '@moeru/eventa'
import { createContext as createWsContext, wsErrorEvent } from '@moeru/eventa/adapters/websocket/native'
import { errorMessageFrom } from '@moeru/std'
import { newMessages, pullMessages, sendMessages } from '@proj-vera/server-sdk-shared'
...
export interface CreateChatWsClientOptions {
  /**
   * Base server URL, e.g. `https://api.vera.build`. The client appends
 ...
 *
 * Before:
 * - "https://api.vera.build", token="abc"
 *
 * After:
 * - "wss://api.vera.build/ws/chat?token=abc"
 *
 * @internal
 */
