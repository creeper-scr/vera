...
export async function createSparkNotifyTools(options: CreateSparkNotifyToolsOptions) {
  ...
  const sparkNoResponseTool = rawTool({
    ...
    execute: async (_rawPayload, context) => {
      ...
      return 'Vera System: Acknowledged, no response or action will be processed.'
    },
  })
  if (allowNoResponse)
  ...
  const sparkCommandTool = rawTool({
    ...
    execute: async (rawPayload, context) => {
      ...
      try {
        ...
        return `Vera System: Error - invalid spark_command parameters: ${errorMessageFrom(error)}`
      }

      return 'Vera System: Acknowledged, command fired.'
    },
  })
