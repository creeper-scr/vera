---
title: 阿里云 NLS
description: 在 Vera 中配置阿里云智能语音交互服务（ASR）
---

阿里云 NLS 为 Vera 提供实时语音转文字（ASR）能力。完成配置后，在“听觉”中选择阿里云 NLS 并测试麦克风输入。
...
凭据验证失败时，请确认三项凭据来自同一阿里云账户与项目，并检查 RAM 用户权限。没有文字结果时，请确认系统已授予 Vera 麦克风权限。
