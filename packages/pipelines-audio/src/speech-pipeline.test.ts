...
describe('createSpeechPipeline', () => {
  ...
  it('reports a terminal cancellation for an intent removed from the pending queue', async () => {
    const { playback } = createPlaybackSpy({ autoEnd: false })
    const pipeline = createSpeechPipeline<string>({
      segmenter: createSegmenter(['speech']),
      playback,
      async tts(request) {
        return request.text
      },
    })
    const cancelled: Array<{ intentId: string, reason?: string }> = []
    pipeline.on('onIntentCancel', event => cancelled.push(event))

    const active = pipeline.openIntent({ intentId: 'active' })
    active.end()
    const pending = pipeline.openIntent({ intentId: 'pending', behavior: 'queue' })
    pending.end()
    pending.cancel('voice-session-stopped')

    await vi.waitFor(() => {
      expect(cancelled).toContainEqual({
        intentId: 'pending',
        reason: 'voice-session-stopped',
      })
    })
  })
