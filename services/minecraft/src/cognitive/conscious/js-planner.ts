...
export interface RuntimeGlobals {
  ...
  notifyVera?: (headline: string, note?: string, urgency?: 'immediate' | 'soon' | 'later') => void
  updateVeraContext?: (text: string, hints?: string[], lane?: string) => void
  history?: unknown
  llmInput?: {
    systemPrompt: string
    userMessage: string
...
export class JavaScriptPlanner {
  ...
  private buildSandboxWorkerRequest(
    ...
    return {
      ...
      bridgeAvailability: {
        ...
        notifyVera: Boolean(globals.notifyVera),
        ...
        updateVeraContext: Boolean(globals.updateVeraContext),
      },
  ...
  private async handleBridgeRequest(method: string, args: unknown[], globals: RuntimeGlobals): Promise<unknown> {
    switch (method) {
      ...
      case 'notifyVera':
        return cloneStructured(globals.notifyVera?.(
          args[0] as string,
          args[1] as string | undefined,
          args[2] as 'immediate' | 'soon' | 'later' | undefined,
        ) ?? null)

      case 'updateVeraContext':
        return cloneStructured(globals.updateVeraContext?.(
          args[0] as string,
          args[1] as string[] | undefined,
  ...
  public describeGlobals(
    ...
    const staticGlobals: Array<Omit<PlannerGlobalDescriptor, 'preview'>> = [
      ...
      { name: 'notifyVera', kind: 'function', readonly: true },
      { name: 'updateVeraContext', kind: 'function', readonly: true },
    ]
    ...
    const valueByName: Record<string, unknown> = {
      ...
      'notifyVera': globals.notifyVera ?? null,
      'updateVeraContext': globals.updateVeraContext ?? null,
      'skip': '[Function skip]',
      'use': '[Function use]',
      'log': '[Function log]',
...
globalThis.forget_conversation = __plannerAvailability.forgetConversation
  ...
  : null
globalThis.notifyVera = __plannerAvailability.notifyVera
  ? (headline, note, urgency) => __plannerCallBridge('notifyVera', [headline, note, urgency])
  : null
globalThis.updateVeraContext = __plannerAvailability.updateVeraContext
  ? (text, hints, lane) => __plannerCallBridge('updateVeraContext', [text, hints, lane])
  : null
