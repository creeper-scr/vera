...
import { isStageCapacitor, isStageTamagotchi } from '@proj-vera/stage-shared'
...
export function useAnalytics() {
  ...
  // ─── Vera card (ccv3 character card) events ──────────────────────────
  // `card_created` is emitted store-side (`stores/modules/vera-card.ts`)
  ...
  /** Pverang QR revealed — the funnel start for `device_channel_connected`. */
  function trackDevicePverangQrShown() {
    if (!canCapture())
      return
    posthog.capture('device_pverang_qr_shown')
  }
  ...
  // ─── Device pverang / channel (Electron / Tamagotchi) ─────────────────
