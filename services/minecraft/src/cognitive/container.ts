...
import { MinecraftContextService as MinecraftContextServiceImpl } from '../vera/minecraft-context-service'
import { VeraBridge as VeraBridgeImpl } from '../vera/vera-bridge'
...
export function createAgentContainer(veraClient: Client) {
  ...
  // Register services
  container.register({
    veraClient: asValue(veraClient),

    veraBridge: asFunction(({ eventBus }: { eventBus: EventBus }) =>
      new VeraBridgeImpl(veraClient, eventBus),
    ).singleton(),

    minecraftContextService: asFunction(({ veraBridge }) =>
    ...
    brain: asClass(Brain)
      ...
      .inject(c => ({
        ...
        veraBridge: c.resolve('veraBridge'),
