import type { WsCloseDetails } from '@proj-vera/better-ws'
import type { WsPeer } from '@proj-vera/better-ws/server'
import type {
  DeliveryConfig,
  ExtensionIdentity,
  ...
  WebSocketBaseEvent,
  WebSocketEvent,
} from '@proj-vera/server-shared/types'
...
import { createServer as createWsServer } from '@proj-vera/better-ws/server'
import { toH3Handler } from '@proj-vera/better-ws/server/h3'
import {
  createInvalidJsonServerErrorMessage,
  ServerErrorMessages,
} from '@proj-vera/server-shared'
import {
  MessageHeartbeat,
...
import {
  ...
  parseEvent,
  stringifyEvent,
} from './server-ws/vera/codec'
import {
  createConsumerOrchestrator,
  isConsumerDeliveryMode,
  normalizeConsumerMode,
  normalizeConsumerPriority,
} from './server-ws/vera/consumers'
import {
  resolveHealthCheckIntervalMs,
  serverWsDefaultHeartbeatTtlMs,
} from './server-ws/vera/liveness'
import {
  createEventMetadata,
  createResponses,
} from './server-ws/vera/responses'
import {
  forEachEventMiddlewares,
  resolveEventDelivery,
} from './server-ws/vera/routing'

interface VeraWsMessage {
  text: () => string
}

interface VeraWsPeerState {
  rawPeer: CrossWsPeer
}

function veraPeerFromRaw(rawPeer: CrossWsPeer): Peer {
  // CrossWS peers expose the connection fields Vera historically used directly
  // (`id`, `send`, `close`, `remoteAddress`, and `request`). Keep the cast in
  // this adapter boundary so protocol code below still depends on the Vera peer
  // contract instead of the concrete transport type.
  return rawPeer as Peer
}

function rawPeerFrom(wsPeer: WsPeer<VeraWsMessage, VeraWsPeerState>): Peer | undefined {
  const rawPeer = wsPeer.state?.rawPeer
  if (!rawPeer) {
    return undefined
  }

  return veraPeerFromRaw(rawPeer)
}
 ...
 *
 * Use when:
 * - Embedding the Vera websocket runtime inside a server process
...
export function setupApp(options?: AppOptions): { app: H3, closeAllPeers: () => void, dispose: () => void } {
  ...
  const appLogger = useLogg('@proj-vera/server-runtime').withLogLevel(logLevelStringToLogLevelMap[appLogLevel]).withFormat(appLogFormat)
  const logger = useLogg('@proj-vera/server-runtime:websocket').withLogLevel(logLevelStringToLogLevelMap[websocketLogLevel]).withFormat(websocketLogFormat)
  ...
  // primitives into `@proj-vera/better-ws/server` so server-runtime only owns
  // Vera authentication, registry sync, route policy, and extension events.
  ...
  // Handles Vera peer lifecycle: open, message, error, close.
  const wsServer = createWsServer<VeraWsMessage, VeraWsPeerState>({
    peers: {
  ...
  wsServer.onPeerHealthChange(({ peer, healthy, silentFor }) => {
    ...
    // Vera runtime peer state still exposes the legacy missedHeartbeats field.
  ...
  app.get('/ws', toH3Handler(wsServer, {
    ...
    error({ peer, rawPeer, error }) {
      handlePeerError(peer ? rawPeerFrom(peer) ?? veraPeerFromRaw(rawPeer) : veraPeerFromRaw(rawPeer), error)
    },
  }))
