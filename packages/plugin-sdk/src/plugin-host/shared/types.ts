...
} from '@proj-vera/plugin-protocol/types'
...
export interface ExtensionManifestV1 {
  ...
  apiVersion: 'v1'
  /** Manifest kind discriminator used to identify Vera extension manifests. */
  kind: 'manifest.extension.vera.moeru.ai'
 ...
 *
 * Use when:
 * - Parsing `extension.vera.json` before loading an extension into the host
 *
 * Expects:
 ...
 *
 * Returns:
 * - A Valibot schema for the Vera extension manifest format
 */
export const extensionManifestV1Schema = object({
  apiVersion: literal('v1'),
  kind: literal('manifest.extension.vera.moeru.ai'),
  id: string(),
