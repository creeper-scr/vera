...
import { Alert } from '@proj-vera/stage-ui/components'
import { importVeraCardPackage, VeraCardPackageError } from '@proj-vera/stage-ui/services/vera-card-import-export'
...
import { InputFileCard } from '@proj-vera/ui'
...
watch(inputFiles, async (newFiles) => {
  ...
  try {
    addCard(await importVeraCardPackage({ file, displayModelsStore }), 'import')
    toast(t('settings.pages.card.imported'))
  }
  catch (error) {
    console.error('Error processing card package:', error)
    toast(t(error instanceof VeraCardPackageError && error.code === 'missing-file'
...
function getModuleShortName(id: string, module: 'consciousness' | 'voice') {
  const card = cards.value.get(id)
  if (!card || !card.extensions?.vera?.modules)
    return 'default'

  const veraExt = card.extensions.vera.modules

  if (module === 'consciousness') {
    return veraExt.consciousness?.model ? veraExt.consciousness.model.split('-').pop() || 'default' : 'default'
  }
  else if (module === 'voice') {
    return veraExt.speech?.voice_id || 'default'
  }
