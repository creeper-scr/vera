...
export const useSettingsAudioDevice = defineStore('settings-audio-devices', () => {
  ...
  // Listening consent is process-local. Restoring it from storage would start
  // microphone capture without a visible user action after app launch.
  const audioInputEnabled = shallowRef(false)
  ...
  function initialize() {
    ...
    if (selectedAudioInputPersist.value && audioInputs.value.length > 0 && !hasSelectedInput) {
      selectedAudioInputPersist.value = selectedAudioInputNonPersist.value
    }
